...
void *thread_function(void *arg) {
    ...
    while (__sync_lock_test_and_set(data->spinlock, 1)) {}; // Spinlock erwerben

    clock_gettime(CLOCK_MONOTONIC_RAW, &data->end_times[data->experiment_index]); // Zeit des Erwerbs des Spinlocks erfassen

    // __sync_lock_release(data->spinlock); // Spinlock freigeben
    return NULL;
}
...
void *release_thread_function(void *arg) {
    ...
    clock_gettime(CLOCK_MONOTONIC_RAW, &data->start_times[data->experiment_index]); // Zeit der Freigabe des Spinlocks erfassen
    __sync_lock_release(data->spinlock); // Spinlock freigeben (auf 0 setzen)
    return NULL;
}
...
int main() {
    ...
    volatile int spinlock = 1; // Spinlock initial auf 1 setzen (geschlossen)
    ...
    for (int i = 0; i < NUM_EXPERIMENTS; i++) {
        ...
        spinlock = 1; // Spinlock initial auf 1 setzen (geschlossen)

        pthread_create(&thread2, NULL, thread_function, (void *)&data);
        usleep(5); // Kurze Pause, um sicherzustellen, dass der wartende Thread zuerst läuft
